...
<div id="main">
 ...
 <tr> <td><button data-show="on" onclick="showHideSummers()" > Show/Hide Summers </button> </td> </tr> 
     ...
     //var req = ;
        	...
        	/***************** COMPLETED COURSES BOX *********************/
        		 ...
        		 for(j=0;j<coursesCounting.length;j++)
        		 {
        		    ...
        		    if(j==0) { $(newEl).append(takenBoxEl); }
        		 ...
        		 for(k=0;k<coursesCountingPlanned.length;k++)
        		 {
        		    ...
        		    if(k==0) { $(newEl).append(plannedBoxEl); }
...
function getSemesterName(code) {
  ...
  // but don't need to query database for this for performance reasons
   ...
   switch(code) {
    ...
    case 3: name= "May"; break;
    case 4: name= "Summer 1"; break;
    case 5: name= "Summer II"; break;
    case 6: name= "Summer 8-week"; break;
    default: name= "N/A"; 
   }
   return name;
}
...
function incrementSemester(sem,year,scale) {
   ...
   var nextYear=0;
   switch(sem) {
   case 1: nextSemester= 2; nextYear=++year; break;
   case 6: nextSemester= 1; nextYear=year; break;
   default: nextSemester=++sem; nextYear=year; 
  ...
  if(sourceId.substr(0,1) == "w") //original move coming from the working side-insert
  {
    ...
    //ui.draggable.draggable( 'option', 'revert', false );
     ...
     $.ajax({
      		url: "addPlanItem.php",
      		method: 'POST',
      		data: {programId: programId, courseId: courseId,hours: hours, semesterCode: semesterCode, planYear: planYear,progYear:progYear, reqId: reqId, proposedReqId: proposedReqId},
      		success: function(result){
        		//alert("success");
        		//alert(result);
        		//Build DOM
  ...
  //style the copy of requirement still left on working side
	...
	// var progYear=2014;
     ...
     $.ajax({
      		...
      		method: 'POST',
      		data: { courseId: courseId, studentId: 1, fromSem: fromSemesterCode, fromYear: fromPlanYear, 
